...
float Greedy::execute(Graph& graph,
  ...
  for (int p = 0; p < vecrequest.size(); p++) {
    ...
    ObjDijkstra.getCostByDijkstra(graph.getAdjList(),
                                  qtvertex,
                                  vecrequest[p].getSrc(),
                                  vecrequest[p].getDst(),
                                  p);
...
float Greedy::deletePath(Graph& graph, int path, int qtvertex, int qtrequests) {
  ...
  std::vector<int> path_cp = ObjDijkstra.Paths[path];
...
float Greedy::executeWithRefine(Graph& graph,
  ...
  while (((path_ch > 0) || (min_cost_ch > 0)
    ...
    for (int i = 0; i < vecrequest.size(); i++) {
      ...
      ObjDijkstra.setAllGraphEdgeIncCost
                 (graph, graph.getQtVertex(), vecrequest[i].getQt());
      ObjDijkstra.getCostByDijkstra
                 (graph.getAdjList(),
                  graph.getQtVertex(),
                  vecrequest[i].getSrc(),
                  vecrequest[i].getDst(),
