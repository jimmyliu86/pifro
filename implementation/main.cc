...
int main(int argc, char *argv[]) {
  ...
  cout.precision(50);
  int qt_executions = 10, delay_time = 30, time_for_execution_original = 600,
                                      order = 1;
  time_t TStart, TStop;
      ...
      for(int a = 0; a < igs_k_.size(); a++){
        for(int b = 0; b < igs_beta_.size(); b++){
          ...
          for (int j = 0; j < qt_executions; j++) {
            ...
            d.Sort();
            IGS igs;
            fout << (igs.executeWithGreedyMinor(g, d, time_for_execution_original, igs_k_[a], igs_beta_[b]) / order) << ";\n";
        ...
        for(int b = 0; b < igs_beta_.size(); b++){
          for(int c = 0; c < igs_x_.size(); c++){
            ...
            for (int j = 0; j < qt_executions; j++) {
              ...
              d.Sort();
              IGS igs;
              fout << (igs.executeWithGreedyMinorX(g, d, time_for_execution_original, igs_k_[a], igs_beta_[b], igs_x_[c]) / order) << ";\n";
              cout << "IGS execution " << j
                   << " for " << "'" << net_name << "' finished at " << (time(NULL) - TStart) << " seconds" << endl;
              cout << "Finishing IGS for '" << net_name << "'\n\n";
              Sleep(delay_time);
      ...
      for(int a = 0; a < igs_k_.size(); a++){
        for(int b = 0; b < igs_beta_.size(); b++){
          ...
          for (int j = 0; j < qt_executions; j++) {
            ...
            d.Sort();
            IGS igs;
            fout << (igs.executeWithPSCMinor(g, d, time_for_execution_original, igs_k_[a], igs_beta_[b]) / order) << ";\n";
            cout << "IGS execution " << j
                 << " for " << "'" << net_name << "' finished at " << (time(NULL) - TStart) << " seconds" << endl;
            cout << "Finishing IGS for '" << net_name << "'\n\n";
            Sleep(delay_time);
        ...
        for(int b = 0; b < igs_beta_.size(); b++){
          for(int c = 0; c < igs_x_.size(); c++){
            ...
            for (int j = 0; j < qt_executions; j++) {
              ...
              d.Sort();
              IGS igs;
              fout << (igs.executeWithPSCMinorX(g, d, time_for_execution_original, igs_k_[a], igs_beta_[b], igs_x_[c]) / order) << ";\n";
              cout << "IGS execution " << j
                   << " for " << "'" << net_name << "' finished at " << (time(NULL) - TStart) << " seconds" << endl;
              cout << "Finishing IGS for '" << net_name << "'\n\n";
              Sleep(delay_time);
