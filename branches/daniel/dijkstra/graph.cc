...
Graph::Graph(int n) : forward_star_(n) {
}

Graph::Graph(int n, const list<Arc>& arcs) : forward_star_(n) {
...
float Graph::dijkstra(int s, int t, list<int> *path) {
  ...
  float distance[num_vertices()];
  std::fill(distance, distance + num_vertices(), FLT_MAX);
  distance[s] = 0;

  // Calcula o caminho mais curto.
  Heap* H = HeapInit(num_vertices());
