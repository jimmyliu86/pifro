...
void Graph::PrintWithCost(string output) {
  ofstream fout;
  fout.open(output.c_str(), fstream::app);
  fout << "\n\nGraph solution with edge cost: ";
  fout << endl;
  for (int i = 0; i < qt_vertex_; i++) {
    fout << "[";
    fout << adj_list_[i][0].number_;
    fout << "]";
    for (int x = 1; x < adj_list_[i].size(); x++) {
      fout << "->";
      fout << adj_list_[i][x].cost_;
      fout << "->";
      fout << "[";
      fout << adj_list_[i][x].number_;
      fout << "]";
    }
    fout << endl;
  }
...
void Graph::PrintWithQtRequests(string output) {
  ofstream fout;
  fout.open(output.c_str(), fstream::app);
  fout << "\n\n\Graph solution with wavelengths: ";
  fout << endl;
  for (int i = 0; i < qt_vertex_; i++) {
    fout << "[";
    fout << adj_list_[i][0].number_;
    ...
    for (int x = 1; x < adj_list_[i].size(); x++) {
      fout << "->";
      fout << adj_list_[i][x].qt_requests_ ;
      fout << "->";
      fout << "[";
      fout << adj_list_[i][x].number_;
      fout << "]";
